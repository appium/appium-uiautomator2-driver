...
appium:autoGrantPermissions | Whether to grant all the requested application permissions automatically when a test starts(`true`). The target sdk version must be over 23 and the device under test must be over Android 6 (API level 23) to grant permissions against a pre-installed application. Lower than the target sdk version 22 must be `fullRest: true` to re-install the application with `-g` option for Android 6+ devices. `false` by default
...
grantPermissions | boolean | no | Set to true in order to grant all the permissions requested in the application's manifest automatically after the installation is completed under Android 6+. The target sdk version must be over 23 and the device under test must be over Android 6 (API level 23) to grant permissions against a pre-installed application. Lower than the target sdk version 22 must be `fullRest: true` to re-install the application with `-g` option for Android 6+ devices. false by default | true
