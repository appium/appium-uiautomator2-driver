...
extras | The result of [getExtras](https://developer.android.com/reference/android/view/accessibility/AccessibilityNodeInfo#getExtras()). The value includes all of keys/values by combining them with `;` separator. | `AccessibilityNodeInfo.roleDescription=;AccessibilityNodeInfo.hint=;AccessibilityNodeInfo.chromeRole=rootWebArea;ACTION_ARGUMENT_HTML_ELEMENT_STRING_VALUES=ARTICLE,BLOCKQUOTE,BUTTON,CHECKBOX,COMBOBOX,CONTROL,FOCUSABLE,FRAME,GRAPHIC,H1,H2,H3,H4,H5,H6,HEADING,HEADING_SAME,LANDMARK,LINK,LIST,LIST_ITEM,LIVE,MAIN,MEDIA,RADIO,RADIO_GROUP,SECTION,TABLE,TEXT_FIELD,TEXT_BOLD,TEXT_ITALIC,TEXT_UNDERLINE,TREE,UNVISITED_LINK,VISITED_LINK` |
...
includeExtrasInPageSource | boolean | Whether to include `extras` element attribute in the page source result. Then, XPath locator can find the element by the extras. The value is one string value by combining keys/values in [getExtras](https://developer.android.com/reference/android/view/accessibility/AccessibilityNodeInfo#getExtras()) with `;` separator, thus you may need to find the element with partial matching like `contain` e.g. `driver.find_element :xpath, '//*[contains(@extras, "AccessibilityNodeInfo.roleDescription=")]'`
