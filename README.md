...
is-collection | Whether the element represents a collection container (e.g. a list or grid), based on [getCollectionInfo](https://developer.android.com/reference/android/view/accessibility/AccessibilityNodeInfo#getCollectionInfo()). Only included in the page source if `true`, to keep its size down. Available since uiautomator2 server version 10.5.0 | 'true'
is-collection-item | Whether the element represents an item within a collection container, based on [getCollectionItemInfo](https://developer.android.com/reference/android/view/accessibility/AccessibilityNodeInfo#getCollectionItemInfo()). Only included in the page source if `true`, to keep its size down. Available since uiautomator2 server version 10.5.0 | 'true'
collection-info | A JSON representation of the element's [CollectionInfo](https://developer.android.com/reference/android/view/accessibility/AccessibilityNodeInfo.CollectionInfo), containing `rowCount`, `columnCount`, `isHierarchical` and `selectionMode`. Only retrievable via `getAttribute`; never included in the page source. `null` if the element is not a collection. Available since uiautomator2 server version 10.5.0 | `{"rowCount":5,"columnCount":1,"isHierarchical":false,"selectionMode":0}`
collection-item-info | A JSON representation of the element's [CollectionItemInfo](https://developer.android.com/reference/android/view/accessibility/AccessibilityNodeInfo.CollectionItemInfo), containing `rowIndex`, `columnIndex`, `rowSpan`, `columnSpan`, `isHeading`, `isSelected`, and (API 33+ only) `rowTitle`/`columnTitle`. Only retrievable via `getAttribute`; never included in the page source. `null` if the element is not a collection item. Available since uiautomator2 server version 10.5.0 | `{"rowIndex":2,"columnIndex":0,"rowSpan":1,"columnSpan":1,"isHeading":false,"isSelected":true,"rowTitle":null,"columnTitle":null}`
