...
enableTopmostWindowFromActivePackage | boolean | Whether to get the window with the highest Z-order from the active package, regardless of whether it is focusable or not. By default it is `false` and only the active application window, implying that the window is focusable, is included to the page source.
