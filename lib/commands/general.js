...
commands.execute = async function (script, args) {
  if (script.match(/^mobile\:/)) {
    ...
    if (script === 'scroll' && !this.isWebContext()) {
      let elementId = args[0].element;
      return await this.uiautomator2.jwproxy.command(`/appium/element/${elementId}/scroll_to`, 'POST', {});
    } else if (script === 'viewportScreenshot' && !this.isWebContext()) {
      return await commands.getViewportScreenshot();
    }
    else {
      this.executeCommand(script, args);
    }
  } else {
    this.executeCommand(script, args);
  }
