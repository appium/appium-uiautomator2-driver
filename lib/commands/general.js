...
commands.execute = async function (script, args) {
  if (script.match(/^mobile\:/)) {
    let mobileScript = script.replace(/^mobile\:/, '').trim();
    if (mobileScript === 'scroll') {
      let elementId = args[0].element;
      let elementToId = args[0].elementTo;
      return await this.uiautomator2.jwproxy.command(`/appium/element/${elementId}/scroll_to/${elementToId}`, 'POST', {});
    } else if (mobileScript === 'viewportScreenshot') {
      return await this.getViewportScreenshot();
    }
  }

  throw new Error(`Unable to execute unknown script: '${script}'`);
};
