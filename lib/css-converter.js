...
function toSnakeCase (str) {
  const tokens = str.split('-').map((str) => str.length > 0 ? str.charAt(0).toUpperCase() + str.slice(1).toLowerCase() : '');
  let out = tokens.join('');
...
function assertGetAttrName (css) {
  ...
  for (let [officialAttr, aliasAttrs] of ATTRIBUTE_ALIASES) {
...
/**
 * Add escapes to regex literals
 *
 * @param {string} str
 * @returns {string} 'str' with regex escapes
 */
 ...
 * @param {?string} valueType Type of attribute (must be string or empty)
 * @param {?string} value Value of the attribute
 * @param {?string} operator The operator between value and value type (=, *=, , ^=, $=)
 */
...
function parseAttr (cssAttr) {
  if (cssAttr.valueType && cssAttr.valueType !== 'string') {
    log.errorAndThrow(`Could not parse '${cssAttr.name}=${cssAttr.value}'.` +
      `Unsupported attribute type '${cssAttr.valueType}'. Only 'string' and empty attribute types are supported.`);
  }
...
function parseCssRule (cssRule) {
  ...
  if (nestingOperator && nestingOperator !== ' ') {
    log.errorAndThrow(`'${nestingOperator}' is not a supported combinator. ` +
 ...
 * @param {Object} type Type of CSS object. 'rule', 'ruleset' or 'selectors'
 */
