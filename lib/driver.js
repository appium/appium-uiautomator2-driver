...
class AndroidUiautomator2Driver extends BaseDriver {
  ...
  async initAUT () {
    ...
    if (this.opts.uninstallOtherPackages) {
      let uninstallOtherPackages;
      try {
        uninstallOtherPackages = helpers.parseArray(this.opts.uninstallOtherPackages);
      } catch (e) {
        logger.errorAndThrow(`Could not parse "uninstallOtherPackages" capability: ${e.message}`);
      }
      await helpers.uninstallOtherPackages(uninstallOtherPackages, this.adb);
    }
