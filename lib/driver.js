...
class AndroidUiautomator2Driver extends BaseDriver {
  ...
  async createSession (...args) {
    try {
      ...
      if (this.opts.autoScreenStream) {
        logger.info(`Starting screen recording automatically at session init`);
        if (this.opts.autoScreenStream === true) {
          // if the user just passed in 'true', set it to an empty object so
          // we get all the default opts
          this.opts.autoScreenStream = {};
        }
        await this.mobileStartScreenStreaming(this.opts.autoScreenStream);
      }
