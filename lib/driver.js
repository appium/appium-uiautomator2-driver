...
// the initial access to ADB may fail because the device is not yet
// in the correct state. a short wait clears this up
...
class AndroidUiautomator2Driver extends BaseDriver {
  ...
  async startUiAutomator2Session () {
    ...
    // the initial access to adb may fail for the device being offline
    const apiLevel = await retryInterval(ADB_INITIAL_RETRIES, 100, async () => {
      return await this.adb.getApiLevel();
    });
