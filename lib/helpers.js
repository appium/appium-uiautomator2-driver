...
helpers.parseChromeDriverPort = function (driverArgs) {
  if (_.isEmpty(driverArgs)) {
    return null;
  }
  if (!_.has(driverArgs, 'chromedriver-port')) {
    return null;
  }

  const chromeDriverPort = driverArgs['chromedriver-port'];

  if (!_.isInteger(chromeDriverPort)) {
    throw new Error(`value of key 'chromedriver-port' is not an integer`);
  }
  return chromeDriverPort;
};

helpers.parseChromedriverExecutable = function (driverArgs) {
  if (_.isEmpty(driverArgs)) {
    return null;
  }
  if (!_.has(driverArgs, 'chromedriver-executable')) {
    return null;
  }

  const chromedriverExecutable = driverArgs['chromedriver-executable'];

  if (!_.isString(chromedriverExecutable)) {
    throw new Error(`value of key 'chromedriver-executable' is not a string`);
  }
