...
class UiAutomator2Server {
  ...
  async installServerApk (installRetries) {
    ...
    if (!apkPackage) {
      throw new Error(`Cannot read the version of '${apkPath}'. Does it exist and accessible?`);
    }
  ...
  async getPackageName (apk) {
    const {name} = await this.adb.getApkInfo(apk);
    return name;
  }

  async getAPKVersion (apk) {
    const {versionName} = await this.adb.getApkInfo(apk);
    return versionName;
  }

  async getInstalledPackageVersion (pkg) {
    const {versionName} = await this.adb.getPackageInfo(pkg);
    return versionName;
  }
