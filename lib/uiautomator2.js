...
import { util, logger, tempDir, fs } from 'appium-support';
    ...
    try {
      ...
      if (_.some(await B.all(srcPackagePaths, async (x) => !await helpers.isWriteable(x)))) {
        ...
        log.info(`Some of the server packages at '${srcPackagePaths}' are not writeable. ` +
          ...
          `Consider making these files writeable manually in order to imporove the performance of session startup.`);
