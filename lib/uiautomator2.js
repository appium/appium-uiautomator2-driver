  ...
  async installServerApk (installTimeout = SERVER_INSTALL_RETRIES * 1000) {
    ...
    if (await B.reduce(packagesInfo,
      ...
      await B.each(packagesInfo, async (pkgInfo) => {
        try {
          await this.adb.uninstallApk(pkgInfo.appId);
        } catch (err) {
          logger.warn(`Error uninstalling '${pkgInfo.appId}': ${err.message}`);
          logger.debug('Continuing');
        }
      });
    }
    await B.each(packagesInfo, async (pkgInfo) => await this.adb.installOrUpgrade(pkgInfo.appPath, pkgInfo.appId, {
