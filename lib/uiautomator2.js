...
class UiAutomator2Server {
  ...
  async startSession (caps) {
    ...
    const leftoversCleanupTimeout = 10 * 1000;
    while (retries < maxRetries) {
      let didProcessExit = false;
      log.info(`Waiting up to ${leftoversCleanupTimeout}ms until instrumentation leftovers are cleaned up...`);
      try {
        await waitForCondition(async () => {
          try {
          ...
          waitMs: leftoversCleanupTimeout,
          intervalMs: 500,
        });
      } catch (err) {
        log.warn(`Instrumentation leftovers have not been cleaned up properly after ${leftoversCleanupTimeout}ms.` +
          `Continuing anyway...`);
      }
