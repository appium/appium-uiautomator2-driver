...
class UiAutomator2Server {
  ...
  async installServerApk (installTimeout = SERVER_INSTALL_RETRIES * 1000) {
    ...
    if (await B.reduce(packagesInfo,
                       async (currentValue, pkgInfo) => (await this.checkAndSignCert(pkgInfo[0], pkgInfo[1])) && currentValue,
                       true)) {
      await B.each(packagesInfo, async (pkgInfo) => {
        try {
          await this.adb.uninstallApk(pkgInfo[0]);
        } catch (err) {
          logger.warn(`Error uninstalling '${pkgInfo[0]}': ${err.message}`);
          logger.debug('Continuing');
        }
      });
    }
    await B.each(packagesInfo, async (pkgInfo) => await this.adb.installOrUpgrade(pkgInfo[0], pkgInfo[1], {
