...
class UiAutomator2Server {
  ...
  async startSession (caps) {
    ...
    logger.info(`Starting uiautomator2 server ${UI2_VER} with cmd: adb shell ${cmd.join(' ')}`);
    //this.adb.shell(cmd);
  ...
  async startSessionUsingAdbKit (deviceUDID) {
    logger.info(`Running command: 'adb -s ${deviceUDID} shell am instrument -w io.appium.uiautomator2.server.test/android.support.test.runner.AndroidJUnitRunner'`);
    this.client.shell(deviceUDID, "am instrument -w io.appium.uiautomator2.server.test/android.support.test.runner.AndroidJUnitRunner")
