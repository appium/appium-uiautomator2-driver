...
class UiAutomator2Server {
  ...
  async installServerApk (installTimeout = SERVER_INSTALL_RETRIES * 1000) {
    // It is important to keep bitwise OR here, since both packages should be signed
    if (!(await this.checkAndSignCert(apkPath, SERVER_PACKAGE_ID)) |
        !(await this.checkAndSignCert(testApkPath, SERVER_TEST_PACKAGE_ID))) {
      await B.each([SERVER_PACKAGE_ID, SERVER_TEST_PACKAGE_ID], async (pkg) => {
        try {
          await this.adb.uninstallApk(pkg);
        } catch (err) {
          logger.warn(`Error uninstalling '${pkg}': ${err.message}`);
          logger.debug('Continuing');
        }
      });
    }
    await B.each([
      [apkPath, SERVER_PACKAGE_ID],
      [testApkPath, SERVER_TEST_PACKAGE_ID],
    ], async (pkgInfo) => await this.adb.installOrUpgrade(pkgInfo[0], pkgInfo[1], {
