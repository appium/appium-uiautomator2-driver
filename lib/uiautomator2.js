...
class UiAutomator2Server {
  ...
  async stopInstrumentationProcess (waitForShutdown = false) {
    try {
      ...
      if (waitForShutdown) {
        await new Promise((resolve) => {
          this.instrumentationProcess?.once('exit', resolve);
  ...
  async deleteSession () {
    ...
    // Deleting the session also stops the downstream HTTP server
    // If the below request fails then it would mean the server is already dead
    let shouldWaitForInstrumentationExit = false;
    try {
      await this.jwproxy.command('/', 'DELETE');
      shouldWaitForInstrumentationExit = true;
    } catch (err) {
    ...
    try {
      await this.stopInstrumentationProcess(shouldWaitForInstrumentationExit);
    } catch (err) {
  ...
  async cleanupAutomationLeftovers (strictCleanup = false) {
    ...
    try {
      ...
      if (activeSessionIds.length) {
        ...
        await this._waitUntilNotListening(
          async() => await axios({
  ...
  /**
   * Blocks until UIA2 stops replying to GET /status requests
   * or SERVER_SHUTDOWN_TIMEOUT_MS expires
   *
   * @param {() => Promise<void>} requestFn
   * @returns {Promise<void>}
   */
  async _waitUntilNotListening(requestFn) {
    try {
      await waitForCondition(async () => {
        try {
          await requestFn();
      ...
      this.log.warn(
        `The UIA2 HTTP server has failed to stop within ${SERVER_SHUTDOWN_TIMEOUT_MS}ms timeout. ` +
        `Continuing anyway`
      );
    }
  }
