...
const IPC_CLAIMED_TOPIC_PATTERN = /uiautomator2:sessionUdidClaimed/i;
...
export function assertSessionClaimIpcTraces(log: string): void {
  if (IPC_UNAVAILABLE_PATTERN.test(log)) {
    throw new Error(
      'Appium server log shows driver-instance IPC is unavailable. ' +
        'Use Appium 3.5.0 or newer so AppiumIpc is exported.',
    );
  }
  ...
  if (SESSION_OVERRIDE_CLEANUP_PATTERN.test(log)) {
    throw new Error(
      'Appium server log shows session-override cleanup. ' +
        ...
        'before the UiAutomator2 driver IPC claim handler can run.',
    );
  }
  ...
  if (!IPC_CLAIMED_TOPIC_PATTERN.test(log)) {
    throw new Error(
      'Appium server log is missing uiautomator2:sessionUdidClaimed IPC activity. ' +
